...
type GetUpdatesRequest struct {
	...
	// the call to the getUpdates, so unwanted updates may be received for a short
	// period of time.
...
type SetWebhookRequest struct {
	...
	// call to the setWebhook, so unwanted updates may be received for a short period
	// of time.
	AllowedUpdates []string `json:"allowed_updates,omitempty"`
...
type SendAudioRequest struct {
	...
	// 200 kB in size. A thumbnail's width and height should not exceed 320. Ignored
...
type SendChatActionRequest struct {
	...
	// receive: *typing* for text messages, *upload_photo* for photos, *record_video*
	...
	// *find_location* for location data, *record_video_note* or *upload_video_note*
	// for video notes.
...
// By default, this method guarantees that after the call the user is not a member
// of the chat, but will be able to join it. So if the user is a member of the
// chat they will also be **removed** from the chat. If you don't want this, use
...
// work and must have the 'can_pin_messages' administrator right in a supergroup
// or 'can_edit_messages' administrator right in a channel. Returns *True* on
// success.
...
// be an administrator in the chat for this to work and must have the appropriate
// administrator rights. Use the field *can_set_sticker_set* optionally returned
// in getChat requests to check if the bot can use this method. Returns *True* on
// success.
...
// fields *png_sticker*, *tgs_sticker*, or *webm_sticker*. Returns *True* on
// success.
type CreateNewStickerSetRequest struct {
	...
	// requirements
	TgsSticker json.RawMessage `json:"tgs_sticker,omitempty"`
...
type AnswerInlineQueryRequest struct {
	...
	// to receive more results. Pass an empty string if there are no more results or
	...
	// the switch button. 1-64 characters, only `A-Z`, `a-z`, `0-9`, `_` and `-` are
	// allowed.
	//
